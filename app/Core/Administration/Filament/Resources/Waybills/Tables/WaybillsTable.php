    ...
    public static function configure(Table $table): Table
    {
        return $table
            ...
            ->filters([
                ...
                Filter::make('pickup_point')->schema([Select::make('pickup_point')->options(fn (): array => app(ActiveCompanyOptionScope::class)->apply(Waybill::query())->whereNotNull('pickup_point')->distinct()->orderBy('pickup_point')->pluck('pickup_point', 'pickup_point')->all())])->query(fn (Builder $query, array $data): Builder => $query->when($data['pickup_point'] ?? null, fn (Builder $query, string $value): Builder => $query->where('pickup_point', $value))),
                Filter::make('destination')->schema([Select::make('destination')->options(fn (): array => app(ActiveCompanyOptionScope::class)->apply(Waybill::query())->whereNotNull('destination')->distinct()->orderBy('destination')->pluck('destination', 'destination')->all())])->query(fn (Builder $query, array $data): Builder => $query->when($data['destination'] ?? null, fn (Builder $query, string $value): Builder => $query->where('destination', $value))),
                Filter::make('truck_number')->schema([Select::make('truck_number')->options(fn (): array => app(ActiveCompanyOptionScope::class)->apply(Waybill::query())->whereNotNull('truck_number')->distinct()->orderBy('truck_number')->pluck('truck_number', 'truck_number')->all())])->query(fn (Builder $query, array $data): Builder => $query->when($data['truck_number'] ?? null, fn (Builder $query, string $value): Builder => $query->where('truck_number', $value))),
                Filter::make('driver_name')->schema([Select::make('driver_name')->options(fn (): array => app(ActiveCompanyOptionScope::class)->apply(Waybill::query())->whereNotNull('driver_name')->distinct()->orderBy('driver_name')->pluck('driver_name', 'driver_name')->all())])->query(fn (Builder $query, array $data): Builder => $query->when($data['driver_name'] ?? null, fn (Builder $query, string $value): Builder => $query->where('driver_name', $value))),
            ...
            ->recordActions([
                ViewAction::make(),
                EditAction::make()
                    ->visible(fn (Waybill $record): bool => ! $record->evidenceIsLockedForEditing()),
            ]);
    }
}
