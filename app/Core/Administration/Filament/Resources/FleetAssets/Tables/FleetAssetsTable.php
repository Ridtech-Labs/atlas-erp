            ...
            ->columns([
                ...
                TextColumn::make('current_state')
                    ...
                    ->description(function (FleetAsset $record): string {
                        ...
                        if ($assignment === null) {
                            return 'Live state from the current assignment lifecycle.';
                        }

                        return sprintf('Live assignment: %s', $assignment->job->job_number ?? 'Assigned Job');
                    })
                ...
                TextColumn::make('operational_status')->label('Operational status')->badge()->description('Master asset condition set by Fleet.')->formatStateUsing(fn (FleetAssetOperationalStatus $state): string => $state->label())->color(fn (FleetAssetOperationalStatus $state): string => $state->color()),
                TextColumn::make('updated_at')->since()->label('Updated')->sortable()->toggleable(),
            ])
            ->filters([
                SelectFilter::make('fleet_asset_type_id')->label('Asset type')->options(fn (): array => app(ActiveCompanyOptionScope::class)->apply(FleetAssetType::query())->orderBy('name')->pluck('name', 'id')->all()),
