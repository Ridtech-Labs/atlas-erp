...
class CancelJobAssetAssignmentAction
{
    ...
    public function execute(JobAssetAssignment $assignment, User $actor, ?string $reason = null): JobAssetAssignment
    {
        if (! $actor->hasPermissionTo(PermissionName::FleetAssignmentsManage->value)
            ...
            throw new BusinessException('You are not allowed to cancel this Fleet assignment.', 403);
        }
        ...
        return DB::transaction(function () use ($assignment, $actor, $reason): JobAssetAssignment {
            ...
            if ((string) $assignment->getRawOriginal('status') !== JobAssetAssignmentStatus::Assigned->value) {
                throw new BusinessException('Only an active Fleet assignment can be cancelled.', 422);
            }
            $assignment->forceFill(['status' => JobAssetAssignmentStatus::Cancelled, 'cancelled_at' => now(), 'cancelled_by' => $actor->getKey(), 'cancellation_reason' => filled($reason) ? trim($reason) : null])->save();
            $this->logger->log('job_asset_assignment.cancelled', 'Fleet assignment cancelled', $actor, $assignment, ['tenant_id' => $assignment->tenant_id, 'company_id' => $assignment->company_id, 'job_id' => $assignment->job_id]);
